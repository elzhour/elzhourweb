...
import { registerFcmForUser, setupForegroundListener } from "@/lib/notifications";
...
export default function PlayerDashboard() {
  ...
  useEffect(() => {
    if (user) { registerFcmForUser(user.uid, "player"); setupForegroundListener(); }
  }, [user]);
  ...
  useEffect(() => {
    return onSnapshot(collection(db, "attendance"), (s) => {
          ...
          <motion.div key="attendance" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
            {sessionDates.length === 0 ? (
              ...
              sessionDates.map((date) => {
                const map = attendanceByDate[date] || {};
                  ...
                  <div key={date} className="bg-card border border-border rounded-2xl overflow-hidden">
                    <div className="bg-muted/30 px-4 py-3 border-b border-border">
                      <div className="font-extrabold text-sm">
                        يوم {getArabicDay(date)} الموافق {formatDisplayDate(date)}
                      </div>
                      <div className="text-[11px] text-muted-foreground mt-0.5">{presentCount} حاضر · {absentCount} غائب</div>
                    </div>
                    <div className="p-3 space-y-1">
                      {players.map((player) => {
                        const status = map[player.id];
                        return (
                          <div key={player.id} className="flex items-center gap-3 py-1.5 px-1">
                            ...
                            <div className="flex-1 font-semibold text-sm truncate">{player.firstName} {player.fatherName}</div>
                            <div className={`text-[11px] font-extrabold px-2.5 py-0.5 rounded-full shrink-0 ${status === "present" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : status === "absent" ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400" : "bg-muted text-muted-foreground"}`}>
